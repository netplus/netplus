...
#define __CH_WRITEABLE_CHECK__( outlet, chp)  \
 ...
 \
	...
	void socket_channel::ch_io_begin(fn_io_event_t const& fn_begin_done) {
		...
		void socket_channel::dup(NRP<netp::promise<std::tuple<int, NRP<socket_channel>>>> p, NRP<event_loop> const& LL) {
			if (!L->in_event_loop()) {
				L->schedule([ch=NRP<socket_channel>(this),p,LL]() {
					ch->dup(p, LL);
				});
				return;
			}

			int nfd = NETP_DUP(m_fd);
			if (nfd == -1) {
				p->set(std::make_tuple(netp_socket_get_last_errno(), nullptr));
				return;
			}

			NRP<socket_cfg> cfg_ = netp::make_ref<socket_cfg>();
			cfg_->fd = nfd;
			cfg_->family = m_family;
			cfg_->type = m_type;
			cfg_->proto = m_protocol;
			cfg_->laddr = m_laddr;
			cfg_->raddr = m_raddr;

			cfg_->L = LL;
			cfg_->option = m_option;

			//@todo, save kvals for this option
			cfg_->kvals = default_tcp_keep_alive_vals ;//
			cfg_->sock_buf = {};
			cfg_->bdlimit = m_outbound_limit;

			LL->execute([p,cfg_,nfd]() {
				int rt;
				NRP<socket_channel> so;
				std::tie(rt, so) = create_socket_channel(cfg_);
				if (rt != netp::OK) {
					NETP_CLOSE_SOCKET(nfd);
					p->set(std::make_tuple(rt, so));
					return;
				}
				p->set( std::make_tuple(netp::OK, so) );
			});
		}
} //end of ns