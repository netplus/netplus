	...
	int get_adapters(vector_adapter_t& adapters, int filter ) {
		...
		while (adapter_address) {
			...
			while (pDnsServer) {
				...
				adapter_.dns.push_back(netp::address(*sa_in).ipv4());
			...
			while (pUnicast) {
				if (pUnicast->Address.lpSockaddr->sa_family == AF_INET) {
					...
					adapter_.unicast.push_back({ netp::address(*sa_in).ipv4(), htonl(mask) });
			...
			while (pANYAddress) {
				...
				case AF_INET:
				{
					...
					adapter_.anycast.push_back(netp::address(*sa_in).ipv4());
				}
				break;
				case AF_INET6:
				{
			...
			while (pMULAddress) {
				...
				case AF_INET:
				{
					...
					adapter_.multicast.push_back(netp::address(*sa_in).ipv4());
				}
				break;
				case AF_INET6:
				{
			...
			while (pGWAddress) {
				...
				case AF_INET:
				{
					...
					adapter_.gateway.push_back(netp::address(*sa_in).ipv4());
				}
				break;
				case AF_INET6:
				{
