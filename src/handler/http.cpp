...
namespace netp { namespace handler {
		...
		int http::__http_parse(NRP<netp::http::parser> const& http_parser, NRP<netp::packet> const& income) {

			NETP_ASSERT(http_parser != nullptr);
			NETP_ASSERT(income != nullptr);
			int http_parse_ec = HPE_OK;

			while (income->len() && http_parse_ec == netp::OK && (m_http_parser) /*http parser would be reset during parse phase by http evt listener*/) {
				http_parse_ec = http_parser->parse((char*)income->head(), income->len());
				if (NETP_LIKELY(http_parse_ec == HPE_OK)) {
					income->skip(income->len());
				} else {
					income->skip(http_parser->calc_parsed_bytes((char*)income->head()));
	...
	void http::read(NRP<netp::channel_handler_context> const& ctx, NRP<netp::packet> const& income) {
		...
		//@NOTE
		// m_http_parser works as a close flag
		NRP<netp::http::parser> parser = m_http_parser;
		if (NETP_HTTP_IS_PARSE_ERROR(__http_parse(parser, income))) {
			ctx->close();
		}
		m_ctx_tmp = nullptr;
	}
	...
	{
		(void)p;
		NETP_ASSERT(m_ctx_tmp != nullptr );
		event_broker_any::invoke<fn_http_message_header_t>(E_MESSAGE_HEADER, m_ctx_tmp, message);
		return netp::OK;
	}

	int http::http_on_body(NRP<netp::http::parser> const& p, const char* data, netp::u32_t len) {
		NETP_ASSERT(m_ctx_tmp != nullptr);
		event_broker_any::invoke<fn_http_message_body_t>(E_MESSAGE_BODY, m_ctx_tmp, data,len);
		(void)p;
		return netp::OK;
	}
	...
	int http::http_on_message_complete(NRP<netp::http::parser> const& p) {
		NETP_ASSERT(m_ctx_tmp != nullptr);
		event_broker_any::invoke<fn_http_message_end_t>(E_MESSAGE_END, m_ctx_tmp);
		(void)p;
		return netp::OK;
	}
	...
	int http::http_on_headers_complete_from(NRP<netp::http::parser> const& p, NRP<netp::http::message> const& message)
	{
		...
		event_broker_any::invoke<fn_http_message_header_from_t>(E_MESSAGE_HEADER, m_ctx_tmp, message, m_from_tmp );
		return netp::OK;
	}

	int http::http_on_body_from(NRP<netp::http::parser> const& p, const char* data, netp::u32_t len) {
		event_broker_any::invoke<fn_http_message_body_from_t>(E_MESSAGE_BODY, m_ctx_tmp, data, len, m_from_tmp );
		(void)p;
		return netp::OK;
	}
	...
	int http::http_on_message_from_complete(NRP<netp::http::parser> const& p) {
		...
		event_broker_any::invoke<fn_http_message_end_from_t>(E_MESSAGE_END_FROM, m_ctx_tmp, m_from_tmp );
		(void)p;
