			...
			if (NETP_LIKELY(rt > 0)) {
				for (int j = 0; j < rt; ++j) {
					...
					if (e->filter&EVFILT_READ) {
						...
						NETP_ASSERT(((ctx->flag & (AIO_READ)) && ctx->iofn[AIO_READ] != nullptr), "fd: %d, flag: %d, ec", ctx->fd, ctx->flag, ec);
#endif
						ctx->iofn[AIO_READ](netp::OK);
					}
					if (e->filter&EVFILT_WRITE) {
#ifdef NETP_DEBUG_WATCH_CTX_FLAG
						NETP_ASSERT(((ctx->flag & (AIO_WRITE)) && ctx->iofn[AIO_WRITE] != nullptr), "fd: %d, flag: %d, ec", ctx->fd, ctx->flag, ec);
#endif
						ctx->iofn[AIO_WRITE](netp::OK);
					}
				}
			}
		}
