...
namespace netp {
	...
	namespace bytes_helper {
		...
		namespace bytes_internal {
			...
			_NETP_CONSTEXPR static __NETP_FORCE_INLINE u32_t __write_impl(T v, OutIt const& start, type<u8_t>) {
				*((byte_t*)start) = v;
				return 1;
			}

			template<class T, class OutIt>
			_NETP_CONSTEXPR static __NETP_FORCE_INLINE u32_t __write_impl(T v, OutIt const& start, type<u16_t>) {
				*((byte_t*)start) = ((v >> 8) & 0xff);
				*((byte_t*)start + 1) = (v & 0xff);
				return 2;
			}
			template<class T, class OutIt>
			_NETP_CONSTEXPR static __NETP_FORCE_INLINE u32_t __write_impl(T v, OutIt const& start, type<u32_t>) {
			...
			template<class T, class OutIt>
			_NETP_CONSTEXPR static __NETP_FORCE_INLINE u32_t __write_impl(T v, OutIt const& start, type<u64_t>) {
		...
		struct big_endian {
			...
			template <class T, class OutIt>
			_NETP_CONSTEXPR static __NETP_FORCE_INLINE netp::u32_t write_impl(T val, OutIt const& start) {
#if __NETP_IS_BIG_ENDIAN
				*((T*)start) = val;
				return sizeof(T);
#else
				...
				//}
				return bytes_internal::__write_impl<T, OutIt>(val,start, typename basic_type_width<T>::width_type_t());
#endif
			}
		...
		struct little_endian {
			...
			template <class T, class OutIt>
			static inline netp::u32_t write_impl(T val, OutIt const& start) {
#if __NETP_IS_LITTLE_ENDIAN
				*((T*)(start)) = val;
				return sizeof(T); 
#else
				return bytes_internal::__write_impl<T, ReadIt>(start, typename basic_type_width<T>::width_type_t());
#endif
				//netp::u32_t write_idx = 0;
