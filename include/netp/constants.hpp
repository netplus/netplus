...
const int  E_ERANGE = -34;         /* Result too large */
	...
	//error.h -> linux/error.h -> asm/error.h
	const int E_EPERM								= NETP_NEGATIVE(EPERM); // -1; operation not permitted
	const int E_ENOENT							= NETP_NEGATIVE(ENOENT); // -2;no such file or directory
	const int E_ESRCH								= NETP_NEGATIVE(ESRCH);// -3; //no such process
	const int E_EINTR								= NETP_NEGATIVE(EINTR);//-4; //interrupted system call
	const int E_EIO										= NETP_NEGATIVE(EIO);//-5; //I/O error
	const int E_ENXIO								= NETP_NEGATIVE(ENXIO);//-6; //no such device or address
	const int E_E2BIG									= NETP_NEGATIVE(E2BIG);//-7; //Arg list too long
	const int E_ENOEXEC							= NETP_NEGATIVE(ENOEXEC);//-8; //Exec format error
	const int E_EBADF								= NETP_NEGATIVE(EBADF);// -9; //Bad file number
	const int E_ECHILD								= NETP_NEGATIVE(ECHILD);//-10; //No child processes
	const int E_EAGAIN								= NETP_NEGATIVE(EAGAIN);//-11; //Try again
	const int E_ENOMEM							= NETP_NEGATIVE(ENOMEM);// -12; //Out of memory
	const int E_EACCES								= NETP_NEGATIVE(EACCES);// -13; //Permission denied
	const int E_EFAULT								= NETP_NEGATIVE(EFAULT);//-14; //Bad address
	const int E_ENOTBLK							= NETP_NEGATIVE(ENOTBLK);//-15; //Block device required
	const int E_EBUSY								= NETP_NEGATIVE(EBUSY);//-16; //Device or resource busy
	const int E_EEXIST								= NETP_NEGATIVE(EEXIST);//-17; //File exists
	const int E_EXDEV								= NETP_NEGATIVE(EXDEV);//-18; //Cross-device link
	const int E_ENODEV							= NETP_NEGATIVE(ENODEV);// -19; //No such device
	const int E_ENOTDIR							= NETP_NEGATIVE(ENOTDIR);//-20; //Not a directory
	const int E_EISDIR								= NETP_NEGATIVE(EISDIR);//-21; //Is a directory
	const int E_EINVAL								= NETP_NEGATIVE(EINVAL);//-22; //Invalid argument
	const int E_ENFILE								= NETP_NEGATIVE(ENFILE);//-23; //File table overflow
	const int E_EMFILE								= NETP_NEGATIVE(EMFILE);//-24; //Too many open files
	const int E_ENOTTY								= NETP_NEGATIVE(ENOTTY);//-25; //Not a tty device
	const int E_ETXTBSY							= NETP_NEGATIVE(ETXTBSY);//-26; //Text file busy
	const int E_EFBIG									= NETP_NEGATIVE(EFBIG);// -27; //File too large
	const int E_ENOSPC							= NETP_NEGATIVE(ENOSPC);//-28; //No space left on device
	const int E_ESPIPE								= NETP_NEGATIVE(ESPIPE);//-29; //Illegal seek
	const int E_EROFS								= NETP_NEGATIVE(EROFS);//-30; //read-only file system
	const int E_EMLINK								= NETP_NEGATIVE(EMLINK);//-31; //Too many links
	const int E_EPIPE									= NETP_NEGATIVE(EPIPE);//-32; //broken pipe
	const int E_EDOM								= NETP_NEGATIVE(EDOM);//-33; //Math argument out of domain
	const int E_ERANGE								= NETP_NEGATIVE(ERANGE);//-34; //Math result not representable
	const int E_EDEADLK							= NETP_NEGATIVE(EDEADLK);//-35; //dead lock wolld occur
	const int E_ENAMETOOLONG				= NETP_NEGATIVE(ENAMETOOLONG);//-36; //file too long
	const int E_ENOLCK								= NETP_NEGATIVE(ENOLCK);//-37; //No record locks available
	const int E_ENOSYS								= NETP_NEGATIVE(ENOSYS);// -38; //Function not implemented
	const int E_ENOTEMPTY						= NETP_NEGATIVE(ENOTEMPTY);// -39; //dierctory not empty
	const int E_ELOOP								= NETP_NEGATIVE(ELOOP);//-40; //too many symbolic links encountered
	const int E_EWOULDBLOCK				= NETP_NEGATIVE(EWOULDBLOCK);//-41; //same as EAGAIN
	const int E_ENOMSG							= NETP_NEGATIVE(ENOMSG);//-42; //No message of desired type
	const int E_EIDRM								= NETP_NEGATIVE(EIDRM);//-43; //identifier removed
	const int E_ECHRNG							= NETP_NEGATIVE(ECHRNG);//-44; //channel number out of range
	const int E_EL2NSYNC							= NETP_NEGATIVE(EL2NSYNC);// -45; //level 2 not synchronized
	const int E_EL3HLT								= NETP_NEGATIVE(EL3HLT);//-46; //level 3 halted
	const int E_EL3RST								= NETP_NEGATIVE(EL3RST);//-47; //level3 reset
	const int E_ELNRNG							= NETP_NEGATIVE(ELNRNG);//-48; //Link number out of range
	const int E_EUNATCH							= NETP_NEGATIVE(EUNATCH);//-49; //protocol driver not attached
	const int E_ENOCSI								= NETP_NEGATIVE(ENOCSI);//-50; //No CSI structure available
	const int E_EL2HLT								= NETP_NEGATIVE(EL2HLT);//-51; //Level 2 halted
	const int E_EBADE								= NETP_NEGATIVE(EBADE);// -52; //Invalid exchange
	const int E_EBADR								= NETP_NEGATIVE(EBADR);//-53; //Invalid request descriptor
	const int E_EXFULL								= NETP_NEGATIVE(EXFULL);//-54; //Exchange full
	const int E_ENOANO							= NETP_NEGATIVE(ENOANO);//-55; //No anode
	const int E_EBADRQC							= NETP_NEGATIVE(EBADRQC);// -56; //Invalid request code
	const int E_EBADSLT							= NETP_NEGATIVE(EBADSLT);//-57; //Invalid slot
	const int E_EDEADLOCK						= NETP_NEGATIVE(EDEADLOCK);//-58; //Same as EDEADLK
	const int E_EBFONT								= NETP_NEGATIVE(EBFONT);// -59; //Bad font file format
	const int E_ENOSTR								= NETP_NEGATIVE(ENOSTR);//-60; //No data available
	const int E_ENODATA							= NETP_NEGATIVE(ENODATA);//-61; //No data available
	const int E_ETIME								= NETP_NEGATIVE(ETIME);//-62; //Timer expired
	const int E_ENOSR								= NETP_NEGATIVE(ENOSR);//-63; //Out of streams resources
	const int E_ERROR_NETNAME_DELETED			= -64; //WINDOWS IOCP: The specified network name is no longer available
	const int E_ENONET							= NETP_NEGATIVE(ENONET);//-64; //machine is not on network
	const int E_ENOPKG							= NETP_NEGATIVE(ENOPKG);// -65; //Package not installed
	const int E_EREMOTE							= NETP_NEGATIVE(EREMOTE);// -66; //Object is remote
	const int E_ENOLINK							= NETP_NEGATIVE(ENOLINK);//-67; //Link has been severed
	const int E_EADV									= NETP_NEGATIVE(EADV);//-68; //Advertise error
	const int E_ESRMNT							= NETP_NEGATIVE(ESRMNT);//-69; //Srmount error
	const int E_ECOMM								= NETP_NEGATIVE(ECOMM);// -70; //Communication error on send
	const int E_EPROTO								= NETP_NEGATIVE(EPROTO);//-71; //protocol error
	const int E_EMULTIHOP						= NETP_NEGATIVE(EMULTIHOP);//-72; //Multihop attempted
	const int E_EDOTDOT							= NETP_NEGATIVE(EDOTDOT);//-73; //RFS specific error
	const int E_EBADMSG							= NETP_NEGATIVE(EBADMSG);// -74; //Not a data message
	const int E_EOVERFLOW						= NETP_NEGATIVE(EOVERFLOW);//-75; //Value too large for defined data type
	const int E_ENOTUNIQ						= NETP_NEGATIVE(ENOTUNIQ);//-76; //Name not unique on network
	const int E_EBADFD								= NETP_NEGATIVE(EBADFD);//-77; //File descriptor in bad state
	const int E_EREMCHG							= NETP_NEGATIVE(EREMCHG);//-78; //Remote address changed
	const int E_ELIBACC								= NETP_NEGATIVE(ELIBACC);//-79; //Cannot access a needed shared library
	const int E_ELIBBAD							= NETP_NEGATIVE(ELIBBAD);//-80; //Accessing a corrupted shared library
	const int E_ELIBSCN							= NETP_NEGATIVE(ELIBSCN);//-81; //A .lib section in an .out is corrupted
	const int E_ELIBMAX							= NETP_NEGATIVE(ELIBMAX);//-82; //Linking in too many shared libraries
	const int E_ELIBEXEC							= NETP_NEGATIVE(ELIBEXEC);// -83; //Cannot exec a shared library directly
	const int E_EILSEQ								= NETP_NEGATIVE(EILSEQ);// -84; //Illegal byte sequence
	const int E_ERESTART							= NETP_NEGATIVE(ERESTART);//-85; //Interrupted system call should be restarted
	const int E_ESTRPIPE							= NETP_NEGATIVE(ESTRPIPE);//-86; //Streams pipe error
	const int E_EUSERS								= NETP_NEGATIVE(EUSERS);//-87; //Too many users
	const int E_ENOTSOCK						= NETP_NEGATIVE(ENOTSOCK);//-88; //socket operation on non-socket
	const int E_EDESTADDRREQ				= NETP_NEGATIVE(EDESTADDRREQ);// -89; //Destination address required

	const int E_EMSGSIZE							= NETP_NEGATIVE(EMSGSIZE);//-90; //Message too long
	const int E_EPROTOTYPE						= NETP_NEGATIVE(EPROTOTYPE);// -91; //protocol wrong type for socket
	const int E_ENOPROTOOPT				= NETP_NEGATIVE(ENOPROTOOPT);//-92; //protocol not available
	const int E_EPROTONOSUPPORT		= NETP_NEGATIVE(EPROTONOSUPPORT);//-93; //protocol not supported
	const int E_ESOCKTNOSUPPORT		= NETP_NEGATIVE(ESOCKTNOSUPPORT);//-94; //socket type not supported
	const int E_EOPNOTSUPP					= NETP_NEGATIVE(EOPNOTSUPP);//-95; //Operation not supported on transport
	const int E_EPFNOSUPPORT				= NETP_NEGATIVE(EPFNOSUPPORT);//-96; //protocol family not supported
	const int E_EAFNOSUPPORT				= NETP_NEGATIVE(EAFNOSUPPORT);//-97; //address family not supported by protocol
	const int E_EADDRINUSE					= NETP_NEGATIVE(EADDRINUSE);//-98; //address already in use
	const int E_EADDRNOTAVAIL				= NETP_NEGATIVE(EADDRNOTAVAIL);//-99; //Cannot assign requested address
	const int E_ENETDOWN						= NETP_NEGATIVE(ENETDOWN);//-100; //Network is down
	const int E_ENETUNREACH					= NETP_NEGATIVE(ENETUNREACH);//-101; //Network is unreachable
	const int E_ENETRESET						= NETP_NEGATIVE(ENETRESET);// -102; //Network dropped
	const int E_ECONNABORTED				= NETP_NEGATIVE(ECONNABORTED);//-103; //Software caused connection
	const int E_ECONNRESET					= NETP_NEGATIVE(ECONNRESET);//-104; //Connection reset by
	const int E_ENOBUFS							= NETP_NEGATIVE(ENOBUFS);//-105; //No buffer space available
	const int E_EISCONN							= NETP_NEGATIVE(EISCONN);//-106; //Transport endpoint
	const int E_ENOTCONN						= NETP_NEGATIVE(ENOTCONN);// -107; //Transport endpoint
	const int E_ESHUTDOWN					= NETP_NEGATIVE(ESHUTDOWN);//-108; //Cannot send after transport
	const int E_ETOOMANYREFS				= NETP_NEGATIVE(ETOOMANYREFS);//-109; //Too many references
	const int E_ETIMEDOUT						= NETP_NEGATIVE(ETIMEDOUT);//-110; //Connection timed
	const int E_ECONNREFUSED				= NETP_NEGATIVE(ECONNREFUSED);// -111; //Connection refused
	const int E_EHOSTDOWN					= NETP_NEGATIVE(EHOSTDOWN);// -112; //Host is down
	const int E_EHOSTUNREACH				= NETP_NEGATIVE(EHOSTUNREACH);//-113; //No route to host
	const int E_EALREADY							= NETP_NEGATIVE(EALREADY);//-114; //Operation already
	const int E_EINPROGRESS					= NETP_NEGATIVE(EINPROGRESS);//-115; //Operation now in
	const int E_ESTALE								= NETP_NEGATIVE(ESTALE);// -116; //Stale NFS file handle
	const int E_EUCLEAN							= NETP_NEGATIVE(EUCLEAN);// -117; //Structure needs cleaning
	const int E_ENOTNAM						= NETP_NEGATIVE(ENOTNAM);// -118; //Not a XENIX-named
	const int E_ENAVAIL							= NETP_NEGATIVE(ENAVAIL);// -119; //No XENIX semaphores
	const int E_EISNAM								= NETP_NEGATIVE(EISNAM);//-120; //Is a named type file
	const int E_EREMOTEIO						= NETP_NEGATIVE(EREMOTEIO);// -121; //Remote I/O error
	const int E_EDQUOT							= NETP_NEGATIVE(EDQUOT);//-122; //Quota exceeded
	const int E_ENOMEDIUM					= NETP_NEGATIVE(ENOMEDIUM);//-123; //No medium found
	const int E_EMEDIUMTYPE					= NETP_NEGATIVE(EMEDIUMTYPE);//-124; //Wrong medium type
	const int E_ECANCELED						= NETP_NEGATIVE(ECANCELED);// -125;
	const int E_ENOKEY								= NETP_NEGATIVE(ENOKEY);// -126;
	const int E_EKEYEXPIRED					= NETP_NEGATIVE(EKEYEXPIRED);//-127;
	const int E_EKEYREVOKED					= NETP_NEGATIVE(EKEYREVOKED);// -128;
	const int E_EKEYREJECTED					= NETP_NEGATIVE(EKEYREJECTED);//-129;
	const int E_EOWNERDEAD					= NETP_NEGATIVE(EOWNERDEAD);//-130;
	const int E_ENOTRECOVERABLE			= NETP_NEGATIVE(ENOTRECOVERABLE);// -131;
	const int E_ERFKILL								= NETP_NEGATIVE(ERFKILL);//-132;
	const int E_EHWPOISON						= NETP_NEGATIVE(EHWPOISON);//-133;
